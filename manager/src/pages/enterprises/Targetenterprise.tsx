...
const [totalPages, setTotalPages] = useState(1);
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '目标企业');

    // 导出工作簿为 Excel 文件
    XLSX.writeFile(wb, '目标企业.xlsx');
  };
  ...
  // 加载企业数据
  const loadEnterprises = async (page = currentPage) => {
  try {
    setLoading(true);
   ...
   const transformedEnterprises = response.data.list.map(transformEnterpriseFromBackend);
      ...
      setTotalItems(response.data.total); // 后端返回总条数
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-xl">目标企业</CardTitle>
            ...
            <div className="flex space-x-2 w-full sm:w-auto justify-end">
              {/* <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={() => loadEnterprises()}>
                    全部
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={async () => {
                    try {
                      const response = await api.getCompanies({ status: 'active' });
                      if (response.success && response.data) {
                        const transformedEnterprises = response.data.map(transformEnterpriseFromBackend);
                        setEnterprises(transformedEnterprises);
                      }
                    } catch (error) {
                  ...
                  <DropdownMenuItem onClick={async () => {
                    try {
                      const response = await api.getCompanies({ status: 'inactive' });
                      if (response.success && response.data) {
                        const transformedEnterprises = response.data.map(transformEnterpriseFromBackend);
                        setEnterprises(transformedEnterprises);
                      }
                  ...
                  <DropdownMenuItem onClick={async () => {
                    try {
                      const response = await api.getCompanies({ status: 'pending' });
                      if (response.success && response.data) {
                        const transformedEnterprises = response.data.map(transformEnterpriseFromBackend);
                        setEnterprises(transformedEnterprises);
                      }
